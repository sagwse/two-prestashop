...
<div class="product-gallery" id="js-product-gallery">
  ...
  {if $images|@count > 1}
    <div class="product-gallery__thumbs" aria-label="{l s='Мініатюри фото товару' d='Shop.Theme.Catalog'}">
      {foreach from=$images item='image' name='thumbs_loop'}
        <button class="product-gallery__thumb{if $image.id_image == $cover_id} product-gallery__thumb--active{/if}"
          type="button" data-image-id="{$image.id_image|intval}"
  ...
  <div class="product-gallery__main" id="js-product-gallery-main">
    ...
    {if $main_img_url}
      <div class="product-gallery__main-wrapper" data-action="open-lightbox" role="button" tabindex="0"
        aria-label="{l s='Відкрити повноекранний перегляд фото' d='Shop.Theme.Catalog'}">
        <img class="product-gallery__main-img" id="js-main-product-img" src="{$main_img_url|escape:'html':'UTF-8'}"
          alt="{$product.name|escape:'html':'UTF-8'}" width="800" height="800" loading="eager" fetchpriority="high">
      </div>
    {else}
        ...
        {foreach from=$images item='image' name='mobile_loop'}
          <div class="product-gallery__mobile-slide" data-slide-index="{$smarty.foreach.mobile_loop.index}">
            <img
              ...
              width="800" height="800" {if $smarty.foreach.mobile_loop.first}loading="eager" fetchpriority="high"
              {else}loading="lazy" 
              {/if}>
          </div>
        {/foreach}
      ...
      {if $images|@count > 1}
        <div class="product-gallery__dots" aria-hidden="true">
          {foreach from=$images item='image' name='dots_loop'}
            <span class="product-gallery__dot{if $smarty.foreach.dots_loop.first} product-gallery__dot--active{/if}"
              data-dot-index="{$smarty.foreach.dots_loop.index}"></span>
          {/foreach}
        </div>
      {/if}
...
<dialog class="product-lightbox" id="js-product-lightbox"
  aria-label="{l s='Повноекранний перегляд фото' d='Shop.Theme.Catalog'}">
  <div class="product-lightbox__inner">
    ...
    {if $images|@count > 1}
      <button class="product-lightbox__prev" type="button" aria-label="{l s='Попереднє фото' d='Shop.Theme.Catalog'}"
        data-lightbox-prev>
    ...
    <div class="product-lightbox__stage">
      <img class="product-lightbox__img" id="js-lightbox-img" src="" alt="{$product.name|escape:'html':'UTF-8'}">
    </div>

    {* Навигация — следующее фото *}
    {if $images|@count > 1}
      <button class="product-lightbox__next" type="button" aria-label="{l s='Наступне фото' d='Shop.Theme.Catalog'}"
        data-lightbox-next>
