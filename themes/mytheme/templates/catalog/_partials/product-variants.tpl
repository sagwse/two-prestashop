...
{if isset($product.groups) && $product.groups|@count > 0}
  ...
  <div class="product-variants" id="js-product-variants">
    ...
    {foreach from=$product.groups key='id_attribute_group' item='group'}
      ...
      <div class="product-variants__group" data-attribute-group-id="{$id_attribute_group|intval}">
        ...
        <div class="product-variants__options" role="radiogroup"
          ...
          {foreach from=$group.attributes key='id_attribute' item='attribute'}
            ...
            <label
              class="product-variants__pill{if $attribute.selected} product-variants__pill--active{/if}{if $is_unavailable} product-variants__pill--unavailable{/if}">

              <input class="product-variants__input visually-hidden" type="radio" name="group[{$id_attribute_group|intval}]"
                value="{$id_attribute|intval}" {if $attribute.selected} checked{/if} {if $is_unavailable} disabled{/if}
                data-product-attribute="{$id_attribute|intval}" {if isset($attribute.url) && $attribute.url}
                data-attribute-url="{$attribute.url|escape:'html':'UTF-8'}" {/if}>
              ...
              {if $group.group_type === 'color' && isset($attribute.html_color_code) && $attribute.html_color_code}
                <span class="product-variants__pill-color"
                  style="background-color: {$attribute.html_color_code|escape:'html':'UTF-8'}" aria-hidden="true"></span>
              {/if}
    ...
    {* ── Сообщение об ошибке выбора варианта ──────────────────────────────
      ...
      ─────────────────────────────────────────────────────────────────── *}
    <p class="product-variants__error" role="alert" aria-live="assertive" id="js-variant-error" hidden>
